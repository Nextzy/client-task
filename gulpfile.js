...
paths.css_vendor = [
  ...
  paths.modules + 'fullcalendar/dist/fullcalendar.css'
]
...
paths.styles_dependencies = [paths.app + '/styles/dependencies/**/*.scss', themes.current.customVariables]

paths.css = [paths.tmp + 'styles/**/*.css', paths.tmp + 'modules/**/*.css', paths.tmp + 'custom.css']
...
paths.coffee = [paths.app + '**/*.coffee', '!' + paths.app + '**/*.spec.coffee']
...
paths.libs = [
  ...
  paths.modules + 'angular-ui-calendar/src/calendar.js',
  paths.modules + 'fullcalendar/dist/fullcalendar.js',
...
gulp.task('scss-lint', [], function() {
  ...
  return gulp
    ...
    .pipe(
      gulpif(
        !isDeploy,
        cache(scsslint({ endless: true, sync: true, config: '.scss-lint.yml' }), {
...
gulp.task('main-css', function() {
  var _paths = [paths.tmp + 'vendor.css', paths.tmp + 'theme-' + themes.current.name + '.css']
...
gulp.task('styles', function(cb) {
  return runSequence('scss-lint', 'sass-compile', ['app-css', 'vendor-css'], 'main-css', cb)
})

gulp.task('styles-lint', function(cb) {
  return runSequence('scss-lint', 'sass-compile', 'css-lint-app', ['app-css', 'vendor-css'], 'main-css', cb)
})

gulp.task('styles-dependencies', function(cb) {
  return runSequence('clear-sass-cache', 'sass-compile', ['app-css', 'vendor-css'], 'main-css', cb)
})
...
gulp.task('coffee-lint', function() {
  gulp
    .src([paths.app + 'modules/**/*.coffee', '!' + paths.app + 'modules/**/*.spec.coffee'])
    .pipe(
      gulpif(
        !isDeploy,
        cache(coffeelint(), {
...
gulp.task('copy-svg', function() {
  return gulp.src(paths.app + '/svg/**/*').pipe(gulp.dest(paths.distVersion + '/svg/'))
})

gulp.task('copy-theme-svg', function() {
  return gulp.src(themes.current.path + '/svg/**/*').pipe(gulp.dest(paths.distVersion + '/svg/' + themes.current.name))
})

gulp.task('copy-fonts', function() {
  return gulp.src(paths.app + '/fonts/*').pipe(gulp.dest(paths.distVersion + '/fonts/'))
})

gulp.task('copy-theme-fonts', function() {
  return gulp.src(themes.current.path + '/fonts/*').pipe(gulp.dest(paths.distVersion + '/fonts/' + themes.current.name))
})

gulp.task('copy-images', function() {
  return gulp
    .src([paths.app + '/images/**/*', paths.app + '/modules/compile-modules/**/images/*'])
    .pipe(gulpif(isDeploy, imagemin({ progressive: true })))
    .pipe(gulp.dest(paths.distVersion + '/images/'))
})

gulp.task('copy-emojis', function() {
  return gulp.src([__dirname + '/emojis/*']).pipe(gulp.dest(paths.distVersion + '/emojis/'))
})
...
gulp.task('unused-css', ['default'], function() {
  return gulp.src([paths.distVersion + 'js/app.js', paths.tmp + '**/*.html']).pipe(
    utils.unusedCss({
...
gulp.task('deploy', function(cb) {
  runSequence(
    ...
    'delete-old-version',
    'delete-tmp',
    ['copy', 'jade-deploy', 'app-deploy', 'jslibs-deploy', 'link-images', 'compile-themes'],
    cb
  )
})
...
gulp.task('default', function(cb) {
  runSequence(
    'delete-old-version',
    'delete-tmp',
    ['copy', 'styles', 'app-watch', 'jslibs-watch', 'jade-deploy', 'express', 'watch'],
    cb
  )
})
